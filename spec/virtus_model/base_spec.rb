  ...
  describe '#validate' do
    ...
    context SimpleModel do
      ...
      context 'invalid' do
        ...
        it { expect(subject.errors.messages.__getobj__).to include({ name: ["can't be blank"] }) }
      end
    end
    ...
    context ComplexModel do
      ...
      context 'invalid' do
        ...
        it { expect(subject.valid?).to be(false) }
        it { expect(subject.errors.messages.__getobj__).to include(:"model[name]" => ["can't be blank"]) }
        it { expect(subject.errors.messages.__getobj__).to include(:"models[0][name]" => ["can't be blank"]) }
      end
    end
  end
